    ...
    sealed public partial class BlackMagic
    {
        ...
        public byte[] ReadBytes(uint dwAddress, int nSize)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private ushort ReadUShort(uint dwAddress)
        {
        ...
        private ushort ReadUShort(uint dwAddress, bool bReverse)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private short ReadShort(uint dwAddress)
        {
        ...
        /// <returns>Returns the value that was read from memory.</returns>
        /// <remarks>Sometimes one needs to read a value where the most significant bytes is not first (i.e. when reading a network packet from memory).  In this case, one would specify 'true' for the bReverse parameter to get the value in a readable format.</remarks>
        private short ReadShort(uint dwAddress, bool bReverse)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private uint ReadUInt(uint dwAddress)
        {
        ...
        /// <returns>Returns the value that was read from memory.</returns>
        /// <remarks>Sometimes one needs to read a value where the most significant bytes is not first (i.e. when reading a network packet from memory).  In this case, one would specify 'true' for the bReverse parameter to get the value in a readable format.</remarks>
        private uint ReadUInt(uint dwAddress, bool bReverse)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private int ReadInt(uint dwAddress)
        {
        ...
        /// <returns>Returns the value that was read from memory.</returns>
        /// <remarks>Sometimes one needs to read a value where the most significant bytes is not first (i.e. when reading a network packet from memory).  In this case, one would specify 'true' for the bReverse parameter to get the value in a readable format.</remarks>
        private int ReadInt(uint dwAddress, bool bReverse)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private UInt64 ReadUInt64(uint dwAddress)
        {
        ...
        /// <returns>Returns the value that was read from memory.</returns>
        /// <remarks>Sometimes one needs to read a value where the most significant bytes is not first (i.e. when reading a network packet from memory).  In this case, one would specify 'true' for the bReverse parameter to get the value in a readable format.</remarks>
        private UInt64 ReadUInt64(uint dwAddress, bool bReverse)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private Int64 ReadInt64(uint dwAddress)
        {
        ...
        /// <returns>Returns the value that was read from memory.</returns>
        /// <remarks>Sometimes one needs to read a value where the most significant bytes is not first (i.e. when reading a network packet from memory).  In this case, one would specify 'true' for the bReverse parameter to get the value in a readable format.</remarks>
        private Int64 ReadInt64(uint dwAddress, bool bReverse)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private float ReadFloat(uint dwAddress)
        {
        ...
        /// <returns>Returns the value that was read from memory.</returns>
        /// <remarks>Sometimes one needs to read a value where the most significant bytes is not first (i.e. when reading a network packet from memory).  In this case, one would specify 'true' for the bReverse parameter to get the value in a readable format.</remarks>
        private float ReadFloat(uint dwAddress, bool bReverse)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private double ReadDouble(uint dwAddress)
        {
        ...
        /// <returns>Returns the value that was read from memory.</returns>
        /// <remarks>Sometimes one needs to read a value where the most significant bytes is not first (i.e. when reading a network packet from memory).  In this case, one would specify 'true' for the bReverse parameter to get the value in a readable format.</remarks>
        private double ReadDouble(uint dwAddress, bool bReverse)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private object ReadObject(uint dwAddress, Type objType)
        {
            if (!this.m_bProcessOpen || this.m_hProcess == IntPtr.Zero)
                throw new MemoryOperationFailedException("Process is not open for read/write.");
        ...
        /// <exception cref="Exception">Throws general exception on failure.</exception>
        /// <returns>Returns the value that was read from memory.</returns>
        private string ReadUnicodeString(uint dwAddress, int nLength)
        {
        ...
        public T Read<T>(params uint[] Addresses)
        {
            ...
            if (Addresses.Length == 1)
            {
                ...
                if (typeof(T) == typeof(string))
                {
                    ...
                    ret = ReadObject(address, typeof(T));
            }
            else
            {
                uint lastAddr = 0;
        ...
        public TDest[] ConvertArray<TDest>(uint Addr, int Count)
        {
            var destType = typeof(TDest);
            var destTypeSize = Marshal.SizeOf(destType);

            var source = ReadBytes(Addr, destTypeSize * Count);
            if (source == null)
